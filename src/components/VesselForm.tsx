...
interface Props {
  vessel: Vessel;
  onSave: (name: string, branch: string, imo: string) => void;
  onClose: () => void;
}
    ...
    <div style={modal}>
      <div style={{ ...modalBox, width:380 }}>
        ...
        <div style={{ marginBottom:12 }}>
          ...
          <input defaultValue={vessel.branch} onChange={e => { branch = e.target.value; }} placeholder="Например: БЛТФ"
            style={{ width:"100%", padding:"8px 10px", borderRadius:6, border:`1px solid ${T.border}`, background:T.bg2, color:T.text, fontSize:13, boxSizing:"border-box" }} />
        </div>
        <div style={{ marginBottom:16 }}>
          <div style={{ fontSize:11, color:T.text2, marginBottom:3 }}>IMO номер</div>
          <input defaultValue={vessel.imo || ""} onChange={e => { imo = e.target.value; }} placeholder="Например: 9663219"
            style={{ width:"100%", padding:"8px 10px", borderRadius:6, border:`1px solid ${T.border}`, background:T.bg2, color:T.text, fontSize:13, boxSizing:"border-box" }} />
        </div>
        <div style={{ display:"flex", gap:8 }}>
          <button onClick={() => onSave(name, branch, imo)} style={{ flex:1, padding:9, borderRadius:6, border:"none", background:T.accent, color:"#fff", fontWeight:700, cursor:"pointer", fontSize:13 }}>Сохранить</button>
