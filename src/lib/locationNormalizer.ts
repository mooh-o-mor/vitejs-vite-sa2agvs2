...
export function extractLocation(raw: string): string {
  ...
  // 4. Убираем повторный "п. п." в начале (только если второй "п" — аббревиатура, т.е. за ней "." или пробел, но не буква)
  ...
  // 8c. Убираем "/" как разделитель: "/ " перед запятой/концом, " / " между словами
  s = s.replace(/\s*\/\s*(?=,|$)/g, "");   // "Владивосток /, Причал" → "Владивосток, Причал"
  s = s.replace(/\s+\/\s+/g, ", ");         // "Владивосток / Причал" → "Владивосток, Причал"
