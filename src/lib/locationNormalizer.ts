...
const WATER_BODY = /^(керченский пролив|балтийское море|охотское море|берингово море|японское море|каспийское море|черное море|азовское море|мыс\|м\.)/i;

/* ── Слова с которых начинаются детали (перед ними ставим запятую) ── */
const DETAIL_START = /^(пр|причал|терминал|наб|набережная|гр|ул|якорная|плавпричал|ПД|СРП|СРЗ|КФ|МСС|КМРП|ССРЗ|КТПБ|КМН|Лукойл|р-н|з-д|завод|район)/i;
...
const KEEP_PREFIX = /^(п\.|б\.|р\.|я\.|мыс\|м\.)\s*/i;
